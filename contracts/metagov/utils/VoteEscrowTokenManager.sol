...
abstract contract VoteEscrowTokenManager is CoreRef {
    ...
    function setLockDuration(uint256 newLockDuration) external onlyZenRole(ZenRoles.METAGOVERNANCE_TOKEN_STAKING) {
    ...
    function lock() external whenNotPaused onlyZenRole(ZenRoles.METAGOVERNANCE_TOKEN_STAKING) {
    ...
    function exitLock() external onlyZenRole(ZenRoles.METAGOVERNANCE_TOKEN_STAKING) {
        veToken.withdraw();
