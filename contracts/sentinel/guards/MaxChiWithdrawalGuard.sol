...
contract MaxChiWithdrawalGuard is IGuard, CoreRef {
    ...
    PCVGuardian public constant pcvGuardian = PCVGuardian(0x0000000000000000000000000000000000000000);

    /// @notice the minimum amount of underlying which can be withdrawn from a deposit that registers in the guard.
    /// i.e. if the min is 100 CHI but the amount in the contract is 1 CHI, the amountToWithdraw will return 0 and the check will fail
    ...
    function setWithdrawInfo(address deposit, WithdrawInfo calldata withdrawInfo)
        public
        hasAnyOfThreeRoles(ZenRoles.GUARDIAN, ZenRoles.GOVERNOR, ZenRoles.PCV_SAFE_MOVER_ROLE)
    {
    ...
    function getAmountToWithdraw(IPCVDeposit deposit) public view returns (uint256) {
        ...
        uint256 liquidity = chi().balanceOf(withdrawInfos[address(deposit)].liquiditySource);
        if (liquidity == 0) {
            return 0;
        }
