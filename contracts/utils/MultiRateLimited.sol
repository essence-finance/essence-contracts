...
abstract contract MultiRateLimited is RateLimited, IMultiRateLimited {
    ...
    /// @param _maxRateLimitPerSecond maximum amount of CHI that can replenish per second ever, this amount cannot be changed by governance
    ...
    function updateAddress(
        ...
        hasAnyOfTwoRoles(ZenRoles.ADD_MINTER_ROLE, ZenRoles.GOVERNOR)
    {
        if (core().hasRole(ZenRoles.ADD_MINTER_ROLE, msg.sender)) {
            require(
    ...
    function addAddressAsMinter(address rateLimitedAddress)
        external
        override
        onlyZenRole(ZenRoles.ADD_MINTER_ROLE)
    {
    ...
    function addAddressAsMinter(
        ...
        uint112 _rateLimitPerSecond,
        uint112 _bufferCap
    ) external onlyZenRole(ZenRoles.ADD_MINTER_ROLE) {
