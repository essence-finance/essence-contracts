...
contract EssenceDAO is GovernorCompatibilityBravo, GovernorVotesComp, GovernorTimelockCompound {
    ...
    address public constant BACKUP_GOVERNOR = 0x0000000000000000000000000000000000000000;
    ...
    constructor(
        ...
        address guardian
    ) GovernorVotesComp(zen) GovernorTimelockCompound(timelock) Governor("Essence DAO") {
        _guardian = guardian;
    }
    ...
    function __rollback(uint256 eta) external {
        require(msg.sender == _guardian, "EssenceDAO: caller not guardian");
        // Deleting guardian prevents multiple triggers of this function
        _guardian = address(0);

        require(eta <= ROLLBACK_DEADLINE, "EssenceDAO: rollback expired");
        _eta = eta;
    ...
    function __executeRollback() external {
        require(_eta <= block.timestamp, "EssenceDAO: too soon");
        require(_guardian == address(0), "EssenceDAO: no queue");
