...
/** 
    @title Essence Finance's Collateralization Oracle Guardian
...
contract CollateralizationOracleGuardian is CoreRef, Timed {
    ...
    /// @param userCirculatingChi new user CHI value
    /// @dev make sure to pause the CR oracle wrapper or else the set value would be overwritten on next update
    function setCache(uint256 protocolControlledValue, uint256 userCirculatingChi)
        external
        ...
        afterTime
    {
        ...
        uint256 cachedUserChi = oracleWrapper.cachedUserCirculatingChi();
        require(
            calculateDeviationThresholdBasisPoints(userCirculatingChi, cachedUserChi) <= deviationThresholdBasisPoints,
            "CollateralizationOracleGuardian: Cached User CHI exceeds deviation"
        );

        // Set the new cache values
        int256 equity = protocolControlledValue.toInt256() - userCirculatingChi.toInt256();
        oracleWrapper.setCache(protocolControlledValue, userCirculatingChi, equity);
