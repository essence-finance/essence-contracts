...
contract CollateralizationOracleWrapper is Timed, ICollateralizationOracleWrapper, CoreRef {
    ...
    /// @notice cached value of the User Circulating CHI
    uint256 public override cachedUserCirculatingChi;
    ...
    function setValidityDuration(uint256 _validityDuration) external override onlyGuardianOrGovernor {
        _setDuration(_validityDuration);
    }
    ...
    function setCache(
        ...
        uint256 _cachedUserCirculatingChi,
        int256 _cachedProtocolEquity
    ) external override onlyGuardianOrGovernor {
        _setCache(_cachedProtocolControlledValue, _cachedUserCirculatingChi, _cachedProtocolEquity);
    }
    ...
    */
    ...
    function _update() internal returns (bool) {
        ...
        outdated =
            ...
            _isExceededDeviationThreshold(cachedUserCirculatingChi, _userCirculatingChi);

        // only update if valid
        require(_validityStatus, "CollateralizationOracleWrapper: CollateralizationOracle is invalid");

        _setCache(_protocolControlledValue, _userCirculatingChi, _protocolEquity);
    ...
    function _setCache(
        ...
        cachedUserCirculatingChi = _cachedUserCirculatingChi;
    ...
    function read() external view override returns (Decimal.D256 memory collateralRatio, bool validityStatus) {
        collateralRatio = Decimal.ratio(cachedProtocolControlledValue, cachedUserCirculatingChi);
    ...
    function isExceededDeviationThreshold() public view override returns (bool obsolete) {
        (
            uint256 _protocolControlledValue,
            uint256 _userCirculatingChi, // int256 _protocolEquity,
            ,
            bool _validityStatus
        ) = ICollateralizationOracle(collateralizationOracle).pcvStats();
    ...
    /// @return protocolEquity : the difference between PCV and user circulating CHI.
    ///         If there are more circulating CHI than $ in the PCV, equity is 0.
    /// @return validityStatus : the current oracle validity status (false if any
    ///         of the oracles for tokens held in the PCV are invalid, or if
    ///         this contract is paused).
    function pcvStats()
        ...
        )
    {
        ...
        userCirculatingChi = cachedUserCirculatingChi;
        ...
        )
    {
        ...
        (protocolControlledValue, userCirculatingChi, protocolEquity, fetchedValidityStatus) = ICollateralizationOracle(
    ...
    function isPcvDeposit(address _pcvDeposit) external view returns (address) {
        return ICollateralizationOracle(
            collateralizationOracle
        ).isPcvDeposit(_pcvDeposit);
    }
}
