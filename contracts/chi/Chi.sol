...
/// @title CHI stablecoin
contract Chi is IChi, ERC20Burnable, CoreRef {
    ...
    /// @notice Chi token constructor
    /// @param core Essence Core address to reference
    constructor(address core) ERC20("Chi USD", "CHI") CoreRef(core) {
        uint256 chainId;
    ...
    /// @notice burn CHI tokens from caller
    /// @param amount the amount to burn
    function burn(uint256 amount) public override(IChi, ERC20Burnable) {
        super.burn(amount);
        emit Burning(msg.sender, msg.sender, amount);
    }

    /// @notice burn CHI tokens from specified account
    /// @param account the account to burn from
    /// @param amount the amount to burn
    function burnFrom(address account, uint256 amount) public override(IChi, ERC20Burnable) onlyBurner whenNotPaused {
    ...
    /// @notice permit spending of CHI
    /// @param owner the CHI holder
    ...
    function permit(
        ...
        require(deadline >= block.timestamp, "Chi: EXPIRED");
        ...
        require(recoveredAddress != address(0) && recoveredAddress == owner, "Chi: INVALID_SIGNATURE");
